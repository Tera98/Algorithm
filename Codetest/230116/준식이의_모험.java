        ...
        for (int i = 0; i < t; i++) {
            ...
            for (int j = 0; j < k; j++) {
                if (input.charAt(j) == 'L' && startX > 1) {
                    if (list.get(startY - 1).charAt(startX - 2) != '#') startX -= 1;
                } else if (input.charAt(j) == 'R' && startX < m) {
                    if (list.get(startY - 1).charAt(startX) != '#') startX += 1;
                } else if (input.charAt(j) == 'U' && startY > 1) {
                    if (list.get(startY - 2).charAt(startX - 1) != '#') startY -= 1;
                } else if (input.charAt(j) == 'D' && startY < n) {
                    if (list.get(startY).charAt(startX - 1) != '#') startY += 1;
                }
            }
            output.add(String.valueOf(startY) + " " + String.valueOf(startX));
        }
        for (int i = 0; i < t; i++) {
